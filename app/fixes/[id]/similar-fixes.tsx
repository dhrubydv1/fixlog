        ...
        {matches.map((match) => {
          ...
          return (
            <article key={match.id} className="rounded-lg border border-zinc-200 bg-zinc-50/60 p-4">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2"><span className="fl-badge fl-badge-accent">{match.category ?? "Uncategorized"}</span>{match.tags && getTags(match.tags).map((tag) => <span key={tag} className="fl-badge">{tag.startsWith("#") ? tag : `#${tag}`}</span>)}<Link href={`/fixes/${match.id}`} className="ml-auto rounded-md px-2.5 py-1.5 text-sm font-medium text-zinc-700 transition hover:bg-zinc-200 focus:outline-none focus:ring-4 focus:ring-zinc-900/10">View Fix</Link></div>
            </article>
          );
        })}
      </div>
