...
export default function Dashboard({ user, aiSuggestionsConfigured }: DashboardProps) {
  ...
  const [pendingDeleteFix, setPendingDeleteFix] = useState<Fix | null>(null);
  ...
  return (
    <main className="fl-page">
      <SiteHeader workspace userName={user.name} onNewFix={startCreatingFix} onSignOut={handleSignOut} isSigningOut={isSigningOut} />
      ...
      <div className="fl-container">
        ...
        <header className="fl-page-heading">
          <p className="fl-eyebrow">Your workspace</p>
        ...
        {showForm && (
          <form
            ...
            className="fl-panel fl-form"
          >
            <div className="fl-panel-heading flex items-center justify-between gap-4">
              <div>
                  ...
                  <div className="mt-4 grid gap-3">
                    {similarFixes.map((fix) => (
                      <article key={fix.id} className="rounded-lg border border-zinc-200 bg-white p-4 shadow-sm">
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-2"><span className="fl-badge fl-badge-accent">{fix.category ?? "Uncategorized"}</span>{fix.tags && getTags(fix.tags).slice(0, 4).map((tag) => <span key={tag} className="fl-badge">{tag.startsWith("#") ? tag : `#${tag}`}</span>)}<Link href={`/fixes/${fix.id}`} className="ml-auto rounded-md px-2.5 py-1.5 text-sm font-medium text-zinc-700 transition hover:bg-zinc-100 focus:outline-none focus:ring-4 focus:ring-zinc-900/10">View Fix</Link></div>
                      </article>
                    ))}
                  </div>
                )}
              ...
              <section aria-labelledby="ai-suggestions-heading" className="border-b border-blue-100 bg-blue-50/60 px-5 py-5 sm:px-6">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex shrink-0 gap-2">
                    <button type="button" onClick={() => setAiSuggestions(null)} className="fl-button">Discard</button>
                    <button type="button" onClick={applySuggestions} className="fl-button fl-button-primary">Apply suggestions</button>
                  </div>
                </div>
            ...
            <div className="fl-form-body">
              <fieldset className="fl-form-section">
                <legend>01 · What happened?</legend>
                <FormField label="Title" htmlFor="fix-title">
                  <input id="fix-title" type="text" value={title} onChange={(event) => setTitle(event.target.value)} required placeholder="Give this Fix a clear, searchable title" className={inputClassName} />
                </FormField>
                <div className="fl-form-grid">
                  <FormField label="Problem" htmlFor="fix-problem">
                    <textarea id="fix-problem" rows={5} value={problem} onChange={(event) => setProblem(event.target.value)} required placeholder="What happened, and where did it happen?" className={inputClassName} />
                  </FormField>
                  <FormField label="Error message · optional" htmlFor="fix-error-message">
                    <textarea id="fix-error-message" rows={5} value={errorMessage} onChange={(event) => setErrorMessage(event.target.value)} placeholder="Paste the relevant error output" className={`${inputClassName} font-mono text-xs leading-5`} />
                  </FormField>
                </div>
              </fieldset>
              <fieldset className="fl-form-section">
                <legend>02 · How you solved it</legend>
                <FormField label="Cause · optional" htmlFor="fix-cause">
                  <textarea id="fix-cause" rows={3} value={cause} onChange={(event) => setCause(event.target.value)} placeholder="What was behind the problem?" className={inputClassName} />
                </FormField>
                <FormField label="Solution" htmlFor="fix-solution">
                  <textarea id="fix-solution" rows={8} value={solution} onChange={(event) => setSolution(event.target.value)} required placeholder="Document the steps, commands, or changes that solved it." className={inputClassName} />
                </FormField>
              </fieldset>
              <fieldset className="fl-form-section">
                <legend>03 · Keep it organized</legend>
                <div className="fl-form-grid">
                  <FormField label="Category" htmlFor="fix-category">
                    <select id="fix-category" value={category} onChange={(event) => setCategory(event.target.value)} className={inputClassName}>
                      ...
                      {FIX_CATEGORIES.map((categoryOption) => <option key={categoryOption} value={categoryOption}>{categoryOption}</option>)}
                    </select>
                  </FormField>
              ...
              <fieldset className="fl-form-section">
                <legend>04 · Who can see this?</legend>
                <div className="fl-form-grid">
                  <label className="fl-visibility-option">
                    <span className="flex items-center gap-2"><input type="radio" name="fix-visibility" value="PRIVATE" checked={visibility === "PRIVATE"} onChange={() => setVisibility("PRIVATE")} /><span className="text-sm font-semibold">Private</span></span>
                    <span className="mt-2 block text-xs leading-6 text-zinc-500">Only you can access this Fix.</span>
                  </label>
                  <label className="fl-visibility-option">
                    <span className="flex items-center gap-2"><input type="radio" name="fix-visibility" value="PUBLIC" checked={visibility === "PUBLIC"} onChange={() => setVisibility("PUBLIC")} /><span className="text-sm font-semibold">Public</span></span>
                    <span className="mt-2 block text-xs leading-6 text-zinc-500">Other developers can view and discover this Fix.</span>
                  </label>
                </div>
                {visibility === "PUBLIC" && <p role="note" className="rounded-lg border border-blue-100 bg-blue-50 px-4 py-3 text-xs leading-6 text-blue-700"><strong>Before sharing:</strong> remove API keys, passwords, tokens, database credentials, and other secrets.</p>}
              </fieldset>
            </div>

            <div className="flex flex-col-reverse gap-3 border-t border-zinc-200 px-5 py-4 sm:flex-row sm:justify-end sm:px-6">
              <button type="button" onClick={closeForm} disabled={isSaving} className="fl-button">
                Cancel
              </button>
              <button type="submit" disabled={isSaving} className="fl-button fl-button-primary">
        ...
        <section aria-labelledby="recent-fixes-heading">
          ...
          <div className="fl-search-panel mt-5">
            

            <div className="relative">
              <label htmlFor="fix-search" className="sr-only">Search fixes</label>
              <input id="fix-search" type="search" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Search your fixes, errors, or tags…" className="fl-field fl-search-input" />
              <span className="pointer-events-none absolute inset-y-0 left-3 flex items-center text-zinc-400"><SearchIcon /></span>
            </div>

            <div className="fl-search-tools">
              ...
              <div className="flex shrink-0 flex-wrap gap-2">
                ...
                {semanticSearchMatches && (
                  <button type="button" onClick={returnToRegularSearch} className="fl-button">Back to regular search</button>
                )}
                {communitySearchResults && (
                  <button type="button" onClick={returnToRegularCommunitySearch} className="fl-button">Back to regular search</button>
                )}
              </div>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div className="flex items-end">
                {hasActiveFilters && (
                  <button type="button" onClick={clearFilters} className="fl-button w-full">
                    Clear filters
                  </button>
                )}
              </div>
          ...
          {semanticSearchMatches && (
            <section aria-labelledby="ai-search-results-heading" className="mt-5 rounded-xl border border-blue-100 bg-blue-50/50 p-5 shadow-sm sm:p-6">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                <div><p className="text-xs font-semibold uppercase tracking-wide text-blue-700">AI search results</p><h3 id="ai-search-results-heading" className="mt-1 text-lg font-semibold tracking-tight text-zinc-950">Results for “{searchQuery.trim()}”</h3><p className="mt-1 text-sm text-zinc-600">Ranked by meaning from your private FixLog.</p></div>
              ...
              {semanticSearchMatches.length === 0 ? (
                ...
                <div className="mt-4 grid gap-3">
                  {semanticSearchMatches.map((fix) => (
                    <article key={fix.id} className="rounded-lg border border-blue-100 bg-white p-4">
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><h4 className="font-medium text-zinc-950">{fix.title}</h4><p className="mt-1 text-sm leading-6 text-zinc-600">{fix.reason}</p></div><span className="shrink-0 rounded-full bg-blue-100 px-2.5 py-1 text-xs font-semibold text-blue-800">{Math.round(fix.score * 100)}% relevant</span></div>
                      <div className="mt-3 flex flex-wrap items-center gap-2"><span className="fl-badge fl-badge-accent">{fix.category ?? "Uncategorized"}</span>{fix.tags && getTags(fix.tags).slice(0, 4).map((tag) => <span key={tag} className="fl-badge">{tag.startsWith("#") ? tag : `#${tag}`}</span>)}<Link href={`/fixes/${fix.id}`} className="ml-auto rounded-md px-2.5 py-1.5 text-sm font-medium text-zinc-700 transition hover:bg-zinc-100 focus:outline-none focus:ring-4 focus:ring-zinc-900/10">View Fix</Link></div>
                    </article>
                  ))}
                </div>
            ...
            <div className="fl-fix-grid">
              {visibleFixes.length === 0 ? (
                <div className="fl-empty">
                  ...
                  {hasActiveFilters ? (
                    <button type="button" onClick={clearFilters} className="fl-button mt-5">
                      Clear filters
                    </button>
                  ) : (
                    <button type="button" onClick={startCreatingFix} className="fl-button fl-button-primary mt-5">
                      <PlusIcon />
                      Add your first fix
                    </button>
                  )}
                ...
                visibleFixes.map((fix) => (
                  <article key={fix.id} className="fl-fix-card">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="fl-card-preview">{fix.problem}</p>
                      </div>
                      <div className="flex shrink-0 flex-col items-end gap-2">
                        <button
                          ...
                          className="fl-icon-button fl-favorite"
                        >
                        ...
                        <p className="text-[10px] text-zinc-500">Updated {formatUpdatedDate(fix.updatedAt)}</p>
                      </div>
                    </div>

                    {fix.errorMessage && (
                      <div className="fl-card-error">
                    ...
                    <div className="fl-card-footer">
                      <div className="flex flex-wrap gap-2">
                        <span className="fl-badge fl-badge-accent">
                        ...
                        {fix.tags && getTags(fix.tags).slice(0, 4).map((tag) => (
                          <span key={tag} className="fl-badge">
      ...
      <Dialog open={pendingDeleteFix !== null} onClose={() => setPendingDeleteFix(null)} title="Delete this Fix?" danger busy={deletingFixId !== null}>
        <p>“{pendingDeleteFix?.title}” will be permanently deleted. This cannot be undone.</p>
        <div className="fl-dialog-footer"><button type="button" className="fl-button" data-dialog-cancel disabled={deletingFixId !== null} onClick={() => setPendingDeleteFix(null)}>Cancel</button><button type="button" className="fl-button fl-button-danger" disabled={deletingFixId !== null} onClick={() => { if (pendingDeleteFix) void deleteFix(pendingDeleteFix); }}>{deletingFixId !== null ? "Deleting…" : "Delete Fix"}</button></div>
      </Dialog>
    </main>
  );
}
...
function StatisticCard({ label, value, isLoading }: { label: string; value: number; isLoading: boolean }) {
  return <dl className="fl-stat" aria-busy={isLoading}><dt>{label}</dt><dd>{isLoading ? "—" : value}</dd></dl>;
}
